...
def auto_add_paragraphs(text: str) -> str:
    ...
    # 1. 強制在每個【步驟X】前面加空行，後面加換行
    text = re.sub(r'(【步驟\d+】[^\n]*)', r'\n\n\1\n', text)
    
    # 2. 在關鍵標點後加換行
    text = re.sub(r'([：。！？])([^\n])', r'\1\n\n\2', text)
    
    # 3. 在公式說明、生活例子、計算步驟等關鍵詞後面加換行
    text = re.sub(r'(公式說明|生活例子|計算步驟|答案：)([^\n])', r'\1\n\n\2', text)
...
【步驟編號系統 - 必須嚴格遵守】
- 所有計算過程必須使用【步驟1】【步驟2】【步驟3】...格式
- 每個【步驟X】必須獨立一行，步驟之間必須空行
- 禁止把多個步驟連在一起寫
- 根據題目複雜度自動調整步驟數量
- 即使簡單計算也要完整編號
                    ...
                    "messages": [
                        {"role": "system", "content": build_system_prompt("請用清楚步驟直接講解完整解法。", profile_type)},
                        {
                            ...
                            "content": [
                                {
                                    "type": "text",
                                    "text": "這是一張數學題的照片，請先將題目完整轉成文字，然後用繁體中文詳細解題。\n\n**特別注意**：\n1. 【極重要】仔細辨識每個數字和符號，特別是對數的「底數」(下標小數字) 和「真數」\n2. 例如 log₉ 2 的底數是 9，log₃ 4 的底數是 3，請特別小心不要看錯\n3. 如果是圖形規律題，請仔細觀察每個圖形，數清楚元素數量\n4. 列出前3-4項的具體數值並找出規律\n5. 驗證計算結果的正確性\n\n解題步驟要包含：\n【題目內容】請完整且準確地列出辨識的題目\n【解題步驟】公式、代入、計算過程\n【最終答案】明確的答案"
                                },
